...
- 공통 규칙(정보 우선순위, 점진적 입력수집, Governance, 미확정 정보 처리, 채널 공통 사실검증,
...
- `agents/promotion-agent.md`, `skills/*/SKILL.md`, `inputs/`, `knowledge/`는 플랫폼에 관계없이
  동일한 원본을 그대로 사용한다.
...
├── inputs/
│   └── business-info-card.md  # 이번 작업 대상 상품·서비스 사실 SSOT
├── knowledge/
...
| `core/PLAYBOOK.md` | 공통 규칙 SSOT. 정보 우선순위, 점진적 입력수집, Governance, 미확정 정보 처리, 채널 공통 사실검증, 기본 작업 흐름. |
| `agents/promotion-agent.md` | 콘텐츠 제작 요청의 실행 절차. 요청 유형에 따라 어떤 skill을 읽을지의 라우팅 기준. |
| `skills/*/SKILL.md` | 채널/문서 유형별 작성 형식·표현 규칙·이미지 가이드. |
| `inputs/business-info-card.md` | 이번 작업 대상 상품·서비스의 사실 데이터. |
...
| 상품·서비스의 가격·일정·환불 등 사실정보를 바꾸고 싶다 | `inputs/business-info-card.md` |
...
```
Input (inputs/business-info-card.md, knowledge/personal-profile.md)
   │
   ▼
PLAYBOOK (core/PLAYBOOK.md) — 정보 우선순위·입력수집·Governance 적용
   │
   ▼
...
1. **영향 파일 식별** — 바꾸려는 내용이 `core/PLAYBOOK.md`(공통 규칙)인지, `agents/`·`skills/`(실행/표현)인지,
   `inputs/`·`knowledge/`(데이터)인지, `CLAUDE.md`·`AGENTS.md`(진입점)인지 먼저 특정하고, 이 표
