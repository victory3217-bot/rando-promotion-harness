...
v0.3부터는 실제 사용 중 발견되는 문제와 개선 아이디어를 [`Usage Log` 이슈 템플릿](.github/ISSUE_TEMPLATE/usage-log.md)으로
GitHub Issue에 축적한다. 각 로그는 BUG / QUALITY / IDEA로 구분해서 기록하며, 정상 작동 사례까지
모두 기록할 필요는 없다.

일정량의 Usage Log가 쌓이면 그 패턴을 분석해 다음 버전의 개선사항을 결정한다. 사용 중 문제가
발생했다고 바로 Harness 규칙을 고치지 않고, 먼저 Usage Log를 남기는 것을 기본 원칙으로 한다.
